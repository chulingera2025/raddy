...
open_files="$(ulimit -n)"
if [[ "$open_files" != "unlimited" && "$open_files" -lt 200000 ]]; then
    die "ulimit -n must be at least 200000 for the 100K scenarios (current: $open_files)"
fi
...
if (( conntrack_max < 100000 )); then
    die "nf_conntrack_max must be at least 100000 for the 100K scenarios (current: $conntrack_max)"
fi
...
required_ports=$(( (100000 + loadgen_shards - 1) / loadgen_shards ))
if (( port_count < required_ports )); then
    die "ip_local_port_range provides $port_count ports per shard; at least $required_ports are required for 100K"
fi
