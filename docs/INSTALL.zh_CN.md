...
Raddex 的发布提供：**校验过的安装脚本** 与 **手动安装路径**，二者都不依赖 `curl | sudo bash`——脚本先下载、校验 sha256，再安装。
...
1. 在 [Releases](https://github.com/chulingera2025/raddex/releases) 下载对应架构的 `raddex-<arch>.tar.gz` 与 `SHA256SUMS`。
2. 校验：
   ```bash
...
`examples/raddex.service` 是一个开箱即用的 systemd unit：开机自启、`systemctl
...
`CAP_NET_BIND_SERVICE`）。unit 默认配置在 `/etc/raddex/Raddexfile`、证书存于
`/var/lib/raddex/certs`——请按你的布局修改。unit 的 `ExecStart` 参数也是执行
`raddex upgrade` 做零停机二进制升级时必须一致传入的参数。
