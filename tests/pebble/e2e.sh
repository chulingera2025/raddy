#!/usr/bin/env bash
# End-to-end test: raddex auto-provisions a certificate for a named site via
...
# --- raddex config: one named site on 443, one on a non-443 TLS port (A1) ---
...
# --- run raddex under sudo so :443 can bind ---
# RADDEX_RENEW_INTERVAL_SECS=5 makes the M8 renewal scheduler scan every 5s so
# the (90s-valid) Pebble certificate is re-issued within the test window.
sudo RADDEX_RENEW_INTERVAL_SECS=5 "$BIN" run -c "$CONFIG" \
  --cert-dir "$CERT_DIR" \
  --acme-directory "https://localhost:14000/dir" \
  --acme-root-pem "$PEBBLE_CA" \
  >/tmp/raddex_e2e.log 2>&1 &
RADDEX_PID=$!

# --- wait for the certificate to be issued ---
echo "waiting for ACME certificate..."
for _ in $(seq 1 60); do
  if [ -f "$CERT_DIR/raddex.test.pem" ]; then
    echo "certificate issued: $CERT_DIR/raddex.test.pem"
    break
  fi
  sleep 1
done
if [ ! -f "$CERT_DIR/raddex.test.pem" ]; then
  echo "timed out waiting for certificate; raddex log:" >&2
  tail -30 /tmp/raddex_e2e.log >&2 || true
  exit 1
fi

# --- verify HTTPS through raddex with the issued certificate ---
# `curl --cacert` validates the served leaf against the issued chain, so a 200
# proves the TLS listener served the correct ACME certificate for raddex.test.
STATUS=$(curl -s -o /dev/null -w "%{http_code}" --cacert "$CERT_DIR/raddex.test.pem" \
  --resolve raddex.test:443:127.0.0.1 \
  https://raddex.test/ 2>/dev/null)
if [ "$STATUS" = "200" ]; then
...
if [ ! -f "$CERT_DIR/alt.test:8443.pem" ]; then
  echo "FAILED: non-443 certificate was not issued; raddex log:" >&2
  tail -30 /tmp/raddex_e2e.log >&2 || true
  exit 1
fi
...
OLD_ENDDATE=$(openssl x509 -in "$CERT_DIR/raddex.test.pem" -noout -enddate 2>/dev/null)
echo "renewal: initial notAfter: $OLD_ENDDATE"
RENEWED=""
for _ in $(seq 1 40); do
  NEW_ENDDATE=$(openssl x509 -in "$CERT_DIR/raddex.test.pem" -noout -enddate 2>/dev/null)
...
if [ -z "$RENEWED" ]; then
  echo "FAILED: certificate was not renewed within 40s; raddex log:" >&2
  tail -30 /tmp/raddex_e2e.log >&2 || true
  exit 1
fi
