...
//!
//! Each test spawns the real `raddex` binary (via `CARGO_BIN_EXE_raddex`) as a
...
//! retention, and the `raddex check` exit codes.
...
const BIN: &str = env!("CARGO_BIN_EXE_raddex");
...
/// A running `raddex run` subprocess plus the config file it reads.
struct RadRaddex {
    child: Child,
    port: u16,
    config_path: PathBuf,
...
impl RadRaddex {
    /// Spawn `raddex run` with a config generated by `config_for` (which receives
    /// the chosen port).
    fn spawn(config_for: impl Fn(u16) -> String) -> RadRaddex {
    ...
    fn spawn_with_env(config_for: impl Fn(u16) -> String, env: &[(&str, &str)]) -> RadRaddex {
        Self::spawn_with_env_probe(config_for, env, ReadyProbe::Plain)
    }

    /// Spawn with extra CLI arguments (e.g. `--metrics-addr`, `--access-log`).
    fn spawn_with_args(config_for: impl Fn(u16) -> String, extra: &[String]) -> RadRaddex {
    ...
    /// handshake.
    fn spawn_tls(config_for: impl Fn(u16) -> String) -> RadRaddex {
    ...
    fn spawn_tcp(config_for: impl Fn(u16) -> String) -> RadRaddex {
    ...
    fn spawn_udp(config_for: impl Fn(u16) -> String) -> RadRaddex {
        Self::spawn_with_probe(config_for, &[], ReadyProbe::Udp)
    }

    /// Like [`spawn_udp`], with extra CLI arguments.
    fn spawn_udp_with_args(config_for: impl Fn(u16) -> String, extra: &[String]) -> RadRaddex {
    ...
    fn spawn_udp_ipv6(config_for: impl Fn(u16) -> String) -> RadRaddex {
    ...
    fn spawn_with_env_probe(
        ...
        loop {
            ...
            let config_path = std::env::temp_dir().join(format!(
                "raddex_integration_{}_{}.Raddexfile",
                std::process::id(),
                port
            ));
            ...
            let child = cmd.spawn().expect("failed to spawn the raddex binary");
            let mut raddex = RadRaddex {
                child,
                port,
                config_path,
            };
            let ready = match probe {
                ReadyProbe::Plain => raddex.wait_for_ready(),
                ReadyProbe::Tls => raddex.wait_for_ready_tls(),
                ReadyProbe::Tcp => raddex.wait_for_ready_tcp(),
                ReadyProbe::Udp => raddex.wait_for_ready_udp(),
                ReadyProbe::UdpV6 => raddex.wait_for_ready_udp_v6(),
            };
            if ready {
                return raddex;
            }
            // The child exited (bind failure) or never listened; retry.
            let _ = raddex.child.kill();
            let _ = raddex.child.wait();
        }
    }
    ...
    fn wait_for_ready(&mut self) -> bool {
        ...
        while Instant::now() < deadline {
            ...
            // flake a test. The probe omits Host so raddex answers 400 without
    ...
    /// Rewrite the Raddexfile and deliver a SIGHUP.
    fn reload(&mut self, config: &str) {
        std::fs::write(&self.config_path, config).unwrap();
        // SAFETY: self.child.id() is this process's own raddex child.
        unsafe {
...
fn forwards_to_upstream() {
    ...
    let raddex =
        RadRaddex::spawn(|port| format!(":{port} {{\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n"));

    let resp = send_request(raddex.port(), Some("localhost"), "/");
...
fn round_robin_across_upstreams() {
    ...
    let raddex = RadRaddex::spawn(|port| {
        format!(
    ...
    for _ in 0..6 {
        assert_eq!(
            send_request(raddex.port(), Some("localhost"), "/").status,
            200
        );
    }
    assert!(
...
fn https_upstream_is_proxied_with_skip_verify() {
    // An `https://` upstream (spec §5.4) with `tls_skip_verify`: raddex talks TLS
    ...
    wait_until(
        || try_request(raddex.port(), Some("localhost"), "/").is_some(),
...
fn https_upstream_untrusted_cert_yields_502() {
    ...
    // verification fails and raddex answers 502 rather than forwarding.
...
fn websocket_upgrade_is_proxied() {
    // A WebSocket-style upgrade (spec §5.5): raddex forwards the `Connection:
    // upgrade` request, the upstream answers 101, and raddex tunnels bytes
    // bidirectionally.
    let (up_port, _up) = UpgradeEchoUpstream::spawn();
    let raddex =
        RadRaddex::spawn(|port| format!(":{port} {{\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n"));
    let mut stream = TcpStream::connect(("127.0.0.1", raddex.port())).unwrap();
    stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .unwrap();
...
fn write_pem_pair(cert_pem: &str, key_pem: &str) -> (PathBuf, PathBuf) {
    ...
    let stem = format!("raddex-site-cert-{}", std::process::id());
...
fn static_tls_site_serves_configured_certificate_and_proxies() {
    ...
    let raddex = RadRaddex::spawn_tls(|port| {
        format!(
            "localhost:{port} {{\n    tls {} {}\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n",
            cert_path.display(),
            key_path.display()
    ...
    let resp = tls_request(raddex.port(), "localhost", "/", &[]).expect("tls request");
    assert_eq!(resp.status, 200);
    assert_eq!(resp.body, "label=secure");
    // The served leaf is the certificate we configured.
    let served = tls_peer_cert_pem(raddex.port(), "localhost").expect("peer cert");
...
fn tls_internal_serves_a_self_signed_certificate() {
    ...
    assert!(
        tls_peer_cert_pem(raddex.port(), "localhost").is_some(),
...
fn mtls_require_rejects_clients_without_certificate() {
    ...
    let ca_path = std::env::temp_dir().join(format!("raddex-ca-{}.pem", std::process::id()));
    ...
    let raddex = RadRaddex::spawn_tcp(|port| {
        format!(
    ...
    assert!(
        tls_request(raddex.port(), "localhost", "/", &[]).is_err(),
...
fn mtls_require_fails_closed_on_unreadable_ca() {
    ...
    let (up_port, _up) = EchoUpstream::spawn("mtls");
    let ca = rcgen::generate_simple_self_signed(vec!["Test CA".to_string()]).unwrap();
    let ca_path = std::env::temp_dir().join(format!("raddex-ca-del-{}.pem", std::process::id()));
    std::fs::write(&ca_path, ca.cert.pem()).unwrap();
    let raddex = RadRaddex::spawn_tcp(|port| {
        format!(
            "localhost:{port} {{\n    tls internal\n    tls client_auth require {}\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n",
...
fn mtls_optional_accepts_clients_without_certificate() {
    ...
    let ca = rcgen::generate_simple_self_signed(vec!["Test CA".to_string()]).unwrap();
    let ca_path = std::env::temp_dir().join(format!("raddex-ca-opt-{}.pem", std::process::id()));
    std::fs::write(&ca_path, ca.cert.pem()).unwrap();
    let raddex = RadRaddex::spawn_tls(|port| {
        format!(
...
fn h2_negotiated_on_tls_listener() {
    ...
    assert_eq!(
        tls_negotiated_alpn(raddex.port(), "localhost").as_deref(),
        Some("h2")
    );
}
...
fn method_matcher_routes_requests() {
    ...
    let get = send_request_method("GET", raddex.port(), Some("localhost"), "/", &[]);
    assert_eq!(get.status, 200);
    assert_eq!(get.body, "label=A");
    let post = send_request_method("POST", raddex.port(), Some("localhost"), "/", &[]);
...
fn host_matcher_selects_within_catch_all() {
    ...
    let api = send_request(raddex.port(), Some("api.example.com"), "/");
    assert_eq!(api.body, "label=A");
    let www = send_request(raddex.port(), Some("www.example.com"), "/");
...
fn respond_terminal_answers_directly() {
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/health");
    assert_eq!(resp.status, 200);
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/other");
    assert_eq!(resp.status, 502);
}
...
fn error_terminal_returns_status() {
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/boom");
...
fn handle_path_strips_prefix() {
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/api/users/1");
    assert_eq!(resp.status, 200);
...
fn rewrite_modifier_changes_path() {
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/x");
    assert_eq!(resp.status, 200);
...
fn basic_auth_requires_credentials() {
    ...
    let no_auth = send_request(raddex.port(), Some("localhost"), "/");
    ...
    let resp = send_request_hdr(raddex.port(), Some("localhost"), "/", &[bad.as_str()]);
    assert_eq!(resp.status, 401);
    let good = format!(
        "Authorization: Basic {}",
        base64::engine::general_purpose::STANDARD.encode("admin:secret")
    );
    let resp = send_request_hdr(raddex.port(), Some("localhost"), "/", &[good.as_str()]);
    assert_eq!(resp.status, 200);
...
fn forward_auth_delegates_and_rejects() {
    ...
    let ok = send_request(raddex.port(), Some("localhost"), "/");
    assert_eq!(ok.status, 200);
    assert_eq!(ok.body, "label=real");
    let denied = send_request(raddex.port(), Some("localhost"), "/deny");
...
fn rate_limit_keys_on_header_value() {
    ...
    assert_eq!(
        send_request_hdr(raddex.port(), Some("localhost"), "/", &[key1]).status,
        200
    );
    assert_eq!(
        send_request_hdr(raddex.port(), Some("localhost"), "/", &[key1]).status,
    ...
    assert_eq!(
        send_request_hdr(raddex.port(), Some("localhost"), "/", &["X-API-Key: key2"]).status,
        200
    );
}
...
fn env_vars_expand_in_config() {
    ...
    let raddex = RadRaddex::spawn_with_env(
...
fn access_log_common_format_is_written() {
    ...
    let log_path =
        std::env::temp_dir().join(format!("raddex_access_common_{}.log", std::process::id()));
...
fn access_log_off_skips_a_site() {
    ...
    let log_path =
        std::env::temp_dir().join(format!("raddex_access_off_{}.log", std::process::id()));
    let (up_port, _up) = EchoUpstream::spawn("off");
    let args = vec![String::from("--access-log"), log_path.display().to_string()];
    let raddex = RadRaddex::spawn_with_args(
        |port| {
            format!(":{port} {{\n    access_log off\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n")
        },
        &args,
...
fn access_log_off_excludes_respond_terminal() {
    ...
    let log_path = std::env::temp_dir().join(format!(
        "raddex_access_off_respond_{}.log",
        std::process::id()
    ));
    let args = vec![String::from("--access-log"), log_path.display().to_string()];
    let raddex = RadRaddex::spawn_with_args(
...
fn missing_host_returns_400() {
    ...
    wait_until(
        || try_request(raddex.port(), None, "/").is_some(),
        "server to accept requests",
    );
    let resp = send_request(raddex.port(), None, "/");
...
fn unknown_host_returns_404_and_named_host_works() {
    ...
    let unknown = send_request(raddex.port(), Some("other.example.com"), "/");
    assert_eq!(unknown.status, 404);
    let known = send_request(raddex.port(), Some("api.example.com"), "/");
...
fn multi_domain_site_block_routes_all_domains() {
    ...
    for host in ["a.example.com", "b.example.com"] {
        let response = send_request(raddex.port(), Some(host), "/");
...
fn h2c_upstream_is_proxied() {
    ...
    let response = send_request(raddex.port(), Some("localhost"), "/h2c");
    assert_eq!(response.status, 200);
...
fn h2_tls_upstream_is_proxied() {
    ...
    let response = send_request(raddex.port(), Some("localhost"), "/h2");
    assert_eq!(response.status, 200);
...
fn http_listener_accepts_ipv6_clients() {
    let raddex = RadRaddex::spawn(|port| format!(":{port} {{\n    respond 200 ipv6-ok\n}}\n"));
    let response = send_request_ipv6(raddex.port(), Some("localhost"), "/v6");
    assert_eq!(response.status, 200);
...
fn http_proxy_connects_to_an_ipv6_upstream() {
    ...
    let response = send_request_ipv6(raddex.port(), Some("localhost"), "/upstream-v6");
    assert_eq!(response.status, 200);
...
fn wildcard_site_routes_http_and_tls_sni() {
    ...
    let response = tls_request(raddex.port(), "api.example.com", "/wildcard", &[]).unwrap();
    assert_eq!(response.status, 200);
...
fn redirect_expands_placeholders() {
    ...
    let resp = send_request(raddex.port(), Some("example.com:8080"), "/a/b?x=1");
...
fn header_up_reaches_upstream() {
    ...
    // `raddex check`/unit layer. (A full header-echo upstream is overkill here.)
    let resp = send_request(raddex.port(), Some("localhost"), "/");
    assert_eq!(resp.status, 200);
    assert_eq!(resp.body, "label=A");
}
...
fn reverse_proxy_applies_header_down() {
    ...
    let resp = send_request(raddex.port(), Some("localhost"), "/x?y=1");
    assert_eq!(resp.status, 200);
    assert_eq!(
        resp.header("x-proxy"),
        Some("/x?y=1"),
...
fn redir_applies_header_down() {
    ...
    let resp = send_request(raddex.port(), Some("example.com:8080"), "/a");
    assert_eq!(resp.status, 308);
    assert_eq!(
...
fn sighup_reload_swaps_config() {
    ...
    assert_eq!(
        send_request(raddex.port(), Some("localhost"), "/").body,
        "label=A"
    );
    raddex.reload(&format!(
        ":{} {{\n    reverse_proxy 127.0.0.1:{b_port}\n}}\n",
        raddex.port()
    ));
    wait_until(
        || try_request(raddex.port(), Some("localhost"), "/").is_some_and(|r| r.body == "label=B"),
...
fn invalid_reload_keeps_old_config() {
    ...
    raddex.reload("bogus config that cannot parse");
    thread::sleep(Duration::from_millis(300));
    assert_eq!(
        send_request(raddex.port(), Some("localhost"), "/").body,
        "label=A"
...
fn upstream_pool_survives_reload() {
    ...
    let responses = keepalive_requests(raddex.port(), "localhost", 2);
    ...
    // upstream connection.
    let responses = keepalive_requests(raddex.port(), "localhost", 1);
...
#[test]
fn raddex_check_exit_codes() {
    let valid = std::env::temp_dir().join(format!(
        "raddex_check_valid_{}.Raddexfile",
        std::process::id()
    ));
    let invalid = std::env::temp_dir().join(format!(
        "raddex_check_invalid_{}.Raddexfile",
        std::process::id()
    ));
...
fn file_server_serves_static_files() {
    let dir = std::env::temp_dir().join(format!("raddex_fs_a_{}", std::process::id()));
...
fn file_server_compresses_on_accept_encoding() {
    let dir = std::env::temp_dir().join(format!("raddex_fs_b_{}", std::process::id()));
    ...
    // raddex is up (wait_for_ready), but the very first request can transiently
...
fn file_server_skips_compressing_tiny_bodies() {
    ...
    let dir = std::env::temp_dir().join(format!("raddex_fs_tiny_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn streaming_compression_scenario(
    ...
    let raddex = RadRaddex::spawn(config);
    let mut stream = TcpStream::connect(("127.0.0.1", raddex.port())).unwrap();
    stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .unwrap();
...
fn file_server_applies_header_down() {
    let dir = std::env::temp_dir().join(format!("raddex_fs_hd_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn big_static_file(tag: &str) -> (std::path::PathBuf, Vec<u8>) {
    let dir = std::env::temp_dir().join(format!("raddex_fs_{tag}_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn file_server_header_down_applies_to_range_responses() {
    let dir = std::env::temp_dir().join(format!("raddex_fs_rangehd_{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn access_log_writes_json_lines() {
    let (up_port, _up) = EchoUpstream::spawn("A");
    let log_path = std::env::temp_dir().join(format!("raddex_access_{}.log", std::process::id()));
...
fn access_log_client_uses_effective_ip() {
    let (up_port, _up) = EchoUpstream::spawn("A");
    let log_path =
        std::env::temp_dir().join(format!("raddex_access_ip_{}.log", std::process::id()));
    let log_str = log_path.to_string_lossy().into_owned();
    let raddex = RadRaddex::spawn_with_args(
...
fn access_log_ts_is_request_start() {
    ...
    let log_path =
        std::env::temp_dir().join(format!("raddex_access_ts_{}.log", std::process::id()));
    let log_str = log_path.to_string_lossy().into_owned();
    let raddex = RadRaddex::spawn_with_args(
        move |port| format!(":{port} {{\n    reverse_proxy 127.0.0.1:{up_port}\n}}\n"),
        &[String::from("--access-log"), log_str],
    ...
    let before = SystemTime::now()
        ...
        .unwrap()
        .as_millis() as u64;
    let resp = send_request(raddex.port(), Some("localhost"), "/slow");
    assert_eq!(resp.status, 200);
...
/// Read the PID from a pidfile written by `raddex run`.
...
fn upgrade_paths(tag: &str) -> (std::path::PathBuf, std::path::PathBuf, std::path::PathBuf) {
    let dir = std::env::temp_dir();
    (
        dir.join(format!("raddex_upgrade_{tag}.pid")),
        dir.join(format!("raddex_upgrade_{tag}.sock")),
        dir.join(format!("raddex_upgrade_{tag}_certs")),
    )
}
...
fn zero_downtime_upgrade_hands_off_listeners() {
    ...
    let load_thread = std::thread::spawn(move || {
        ...
        while !stop_thread.load(Ordering::Relaxed) {
            ...
            // thread-per-connection upstream, not raddex.
            thread::sleep(Duration::from_micros(1000));
        }
        let _ = result_tx.send((ok, failed));
    });

    // Run `raddex upgrade` (the same binary) with the same flags the instance
    // was started with; it must hand off the listeners and return 0.
    let mut cmd = Command::new(BIN);
    cmd.args(["upgrade", "-c"]).arg(&raddex.config_path);
    cmd.args(&extra);
    cmd.env("RUST_LOG", "error");
    let status = cmd.status().expect("failed to spawn raddex upgrade");
    assert!(
        status.success(),
        "raddex upgrade should succeed: {status:?}"
    );
    ...
    // The replacement is detached from `RadRaddex`'s Drop, so stop it directly.
...
fn upgrade_aborts_on_broken_config_without_disturbing_instance() {
    ...
    std::fs::write(&raddex.config_path, "this is not a valid Raddexfile {\n").unwrap();
...
fn untrusted_peer_ip_hash_ignores_xff() {
    ...
    for n in 0..20 {
        ...
        let resp = send_request_hdr(raddex.port(), Some("localhost"), "/", &[&header]);
        assert_eq!(resp.status, 200);
        bodies.insert(resp.body.clone());
    }
    assert_eq!(
...
fn health_check_recovers_after_upstream_returns() {
    ...
    wait_until(
        || {
            let mut seen = std::collections::BTreeSet::new();
            for _ in 0..10 {
                seen.insert(send_request(raddex.port(), Some("localhost"), "/").body);
            }
    ...
    b.pause();
    wait_until(
        || (0..10).all(|_| send_request(raddex.port(), Some("localhost"), "/").body == "label=A"),
...
fn health_state_survives_reload() {
    let (a_port, _a) = EchoUpstream::spawn("A");
    let (b_port, mut b) = ToggleUpstream::spawn("B");
    let mut raddex = RadRaddex::spawn(|port| {
        format!(
            ":{port} {{\n    {}\n}}\n",
            health_checked_proxy(&[a_port, b_port])
        )
...
// ---------------------------------------------------------------------------
// Migration (`raddex import`, ARCHITECTURE §7)
// ---------------------------------------------------------------------------

#[test]
fn import_caddyfile_output_validates() {
    let src = std::env::temp_dir().join(format!("raddex_import_{}.Caddyfile", std::process::id()));
    let out = std::env::temp_dir().join(format!("raddex_import_{}.Raddexfile", std::process::id()));
    std::fs::write(
        &src,
    ...
    // The converted Raddexfile must pass `raddex check` (the same validation a
    // reload performs).
...
fn import_nginx_output_validates() {
    let src = std::env::temp_dir().join(format!("raddex_import_nginx_{}.conf", std::process::id()));
    let out = std::env::temp_dir().join(format!(
        "raddex_import_nginx_{}.Raddexfile",
        std::process::id()
    ));
    std::fs::write(
        &src,
...
fn raw_tcp_proxy_relays_bidirectionally() {
    ...
    assert_eq!(tcp_echo(raddex.port(), "ping-l4"), "echo:ping-l4");
}
...
fn raw_tcp_proxy_round_robins_across_upstreams() {
    ...
    for _ in 0..4 {
        assert!(
            tcp_echo(raddex.port(), "rr") == "echo:rr",
...
fn raw_tcp_proxy_enforces_max_connections() {
    ...
    let _raddex = RadRaddex::spawn_with_probe(
        |http_port| {
            format!(
                ":{http_port} {{\n    respond 200 ready\n}}\n\
    ...
    wait_until(
        || metric_value(metrics_port, "raddex_l4_tcp_rejected_total").unwrap_or(0) >= 1,
...
fn raw_tcp_proxy_closes_on_failed_upstream_connect() {
    ...
    stream
        ...
        .unwrap();
    wait_until(
        || metric_value(metrics_port, "raddex_l4_tcp_connect_failures_total").unwrap_or(0) >= 1,
...
fn raw_tcp_proxy_reload_updates_upstream_set_for_new_connections() {
    ...
    let (a_port, a) = TcpEchoUpstream::spawn();
    let (b_port, b) = TcpEchoUpstream::spawn();
    let mut raddex = RadRaddex::spawn_tcp(|port| {
        format!("tcp 127.0.0.1:{port} {{\n    to 127.0.0.1:{a_port}\n}}\n")
    });
    assert_eq!(tcp_echo(raddex.port(), "pre"), "echo:pre");
    ...
    wait_until(
        || {
            tcp_echo_opt(raddex.port(), "post").as_deref() == Some("echo:post")
                && b.hit_count() >= 1
        },
...
fn raw_tcp_proxy_sni_routes_by_hostname() {
    ...
    send_sni(raddex.port(), "a.test");
    send_sni(raddex.port(), "b.test");
...
fn raw_tcp_proxy_can_terminate_tls_before_relaying() {
    ...
    assert_eq!(tls_raw_roundtrip(raddex.port(), b"hello"), b"echo:hello");
}
...
fn raw_tcp_proxy_sni_routes_one_label_wildcards() {
    ...
    send_sni(raddex.port(), "api.example.com");
    send_sni(raddex.port(), "api.sub.example.com");
    send_sni(raddex.port(), "deep.api.example.com");
...
fn raw_tcp_proxy_sni_fallback_serves_unmatched() {
    ...
    send_sni(raddex.port(), "a.test");
    send_sni(raddex.port(), "unknown.test");
    assert!(a.hit_count() >= 1, "SNI a.test must route to A");
    assert!(
        fb.hit_count() >= 1,
...
fn raw_udp_proxy_relays_datagrams() {
    ...
    let raddex = RadRaddex::spawn_udp(|port| {
        format!("udp 127.0.0.1:{port} {{\n    to 127.0.0.1:{echo_port}\n}}\n")
    });
    wait_until(
        || udp_roundtrip(raddex.port(), "ping").as_deref() == Some("echo:ping"),
...
fn raw_udp_proxy_supports_ipv6_listeners() {
    ...
    let raddex = RadRaddex::spawn_udp_ipv6(|port| {
        format!("udp [::1]:{port} {{\n    to 127.0.0.1:{echo_port}\n}}\n")
    });
    wait_until(
        || udp_roundtrip_ipv6(raddex.port(), "v6").as_deref() == Some("v6:v6"),
...
fn raw_udp_proxy_supports_ipv6_upstreams() {
    ...
    wait_until(
        || {
            udp_roundtrip(raddex.port(), "v6-upstream").as_deref()
                == Some("v6-upstream:v6-upstream")
        },
...
fn raw_udp_proxy_round_robins_and_pins_by_ip_hash() {
    ...
    let rr = RadRaddex::spawn_udp(|port| {
        format!(
    ...
    let ih = RadRaddex::spawn_udp(|port| {
        format!(
            "udp 127.0.0.1:{port} {{\n    to 127.0.0.1:{a_port} 127.0.0.1:{b_port}\n    lb_policy ip_hash\n}}\n"
        )
    });
...
fn raw_udp_proxy_idle_timeout_evicts_flows() {
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "x").is_some(),
    ...
    wait_until(
        || {
            try_request(metrics_port, None, "/metrics").is_some_and(|r| {
                r.body.lines().any(|l| {
                    l.starts_with("raddex_l4_udp_idle_evictions_total{")
                        && l.split_whitespace()
                            .last()
...
fn raw_udp_proxy_drops_oversized_datagrams() {
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "small").as_deref() == Some("echo:small"),
    ...
    client
        ...
        .unwrap();
    client
        .send_to(b"123456789", ("127.0.0.1", raddex.port()))
        .unwrap();
    let mut buf = [0u8; 64];
    assert!(
    ...
    wait_until(
        || metric_value(metrics_port, "raddex_l4_udp_oversized_drops_total").unwrap_or(0) >= 1,
...
fn raw_udp_proxy_reload_updates_datagram_limit() {
    let (echo_port, _echo) = UdpEchoUpstream::spawn("echo");
    let metrics_port = free_port();
    let mut raddex = RadRaddex::spawn_udp_with_args(
        |port| {
            format!(
                "udp 127.0.0.1:{port} {{\n    to 127.0.0.1:{echo_port}\n    max_datagram_size 8\n}}\n"
            )
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "123456789").as_deref() == Some("echo:123456789"),
...
fn raw_udp_proxy_evicts_flows_at_capacity() {
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "first").as_deref() == Some("echo:first"),
        "the first bounded UDP flow to receive a reply",
    );
    wait_until(
        || udp_roundtrip(raddex.port(), "second").as_deref() == Some("echo:second"),
        "the second bounded UDP flow to receive a reply",
    );
    wait_until(
        || metric_value(metrics_port, "raddex_l4_udp_capacity_evictions_total").unwrap_or(0) >= 1,
...
fn raw_udp_proxy_reload_updates_upstream_for_new_flows() {
    let (a_port, _a) = UdpEchoUpstream::spawn("A");
    let (b_port, _b) = UdpEchoUpstream::spawn("B");
    let mut raddex = RadRaddex::spawn_udp(|port| {
        format!("udp 127.0.0.1:{port} {{\n    to 127.0.0.1:{a_port}\n}}\n")
    });
    wait_until(
        || udp_roundtrip(raddex.port(), "before").as_deref() == Some("A:before"),
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "after").as_deref() == Some("B:after"),
...
fn raw_udp_proxy_writes_typed_flow_access_records() {
    ...
    wait_until(
        || udp_roundtrip(raddex.port(), "logged").as_deref() == Some("echo:logged"),
...
fn udp_listener_conflicts_with_overlapping_udp_bind() {
    ...
    let config = std::env::temp_dir().join(format!(
        "raddex_l4_udp_conflict_{}.Raddexfile",
        std::process::id()
    ));
    std::fs::write(
        &config,
...
fn zero_downtime_upgrade_hands_off_raw_tcp_listeners() {
    ...
    // process: after `raddex upgrade`, new TCP connections are served by the
    ...
    let status = Command::new(BIN)
        .args(["upgrade", "-c"])
        .arg(&raddex.config_path)
        .args(&extra)
        .status()
        .expect("failed to spawn raddex upgrade");
    assert!(
        status.success(),
        "raddex upgrade should succeed: {status:?}"
    );
    ...
    // Stop the replacement (it is detached from `RadRaddex`'s Drop).
    unsafe {
        libc::kill(new_pid, libc::SIGKILL);
    }
    let deadline = Instant::now() + Duration::from_secs(10);
...
fn tcp_listener_conflicts_with_http_site_at_check() {
    // `raddex check` must reject a raw-TCP listener on an HTTP site's port.
    let config = std::env::temp_dir().join(format!(
        "raddex_l4_conflict_{}.Raddexfile",
        std::process::id()
    ));
    std::fs::write(
        &config,
